...
class FeatureLayer(nn.Module):
    ...
    def forward(self, inputs, rois):
        ...
        :param inputs: batch of image (batch_size, c, h, w)
        :param rois: batch of roi (batch_size, num_roi, 4)
        :return:
        """
        vgg16_start = time.time()
        features = self.vgg16_conv(inputs)
...
class FastRCNN(nn.Module):
    def __init__(self, n_classes=81, roipool_size=7):
        ...
        self.feature_layer = FeatureLayer(roipool_size=roipool_size)
        self.rcnn_layer = RCNNLayer((512, roipool_size, roipool_size), n_classes)

    def forward(self, inputs, roi):
        roi_features, vgg16_time, roipool_time = self.feature_layer(inputs, roi)
