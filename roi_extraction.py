...
def selective_search_extraction(img, search_type='fast'):
    """
...
def train_roi_extract(extracted_regions,
                      batch_anns,
    ...
    for batch_idx, (extracted_region, batch_ann) in enumerate(zip(extracted_regions, batch_anns)):
        ious = torch.tensor([[iou(box, bbox_gt) for bbox_gt in batch_ann] for box in extracted_region],
                            dtype=torch.float)
...
def roi_target(roi, roi_label, means_coef, stds_coef, numbered_category):
...
if __name__ == '__main__':
    ...
    sample_anns = torch.tensor([[10, 10, 20, 20, 1],
                                [30, 30, 50, 50, 2],
                                [50, 20, 30, 10, 3]])
    roi, roi_label = train_roi_extract(sample_region, sample_anns, 4)
